...
args_for_compute_gradient::args_for_compute_gradient(int N__, const int M__, int D__, int K__){
	N = N__;
	M = M__;
	D = D__;
	K = K__;
...
void args_for_compute_gradient::SET_parameters(map<string, double> parameters__){
	...
	parameters["num_sep"] = parameters__["num_sep"];
}

args_for_compute_gradient::~args_for_compute_gradient(){
